...
class PoseFusionNode(Node):
    ...
    def get_base_to_map_transform(self):
        try:
            ...
            self.trans_map_odom = self.tf_buffer.lookup_transform('map', 'odom', latest_time, timeout=rclpy.duration.Duration(seconds=1.0))
            
            self.trans_odom_base = self.tf_buffer.lookup_transform('odom', 'base_footprint', latest_time, timeout=rclpy.duration.Duration(seconds=1.0))

            # Return position and orientation
            trans_map_base = [self.trans_map_odom.transform.translation.x + self.trans_odom_base.transform.translation.x, 
                              self.trans_map_odom.transform.translation.y + self.trans_odom_base.transform.translation.y, 
                              self.trans_map_odom.transform.translation.z + self.trans_odom_base.transform.translation.z]
            
            # Calculate combined rotation quaternion
            rot_map_to_odom = [
                0.0,
                0.0,
                self.trans_map_odom.transform.rotation.z,
                self.trans_map_odom.transform.rotation.w
            ]

            rot_odom_to_base = [
                0.0,
                0.0,
                self.trans_odom_base.transform.rotation.z,
                self.trans_odom_base.transform.rotation.w
            ]

            rot_map_base = quaternion_multiply(rot_odom_to_base, rot_map_to_odom)
    ...
    def publish_tf(self):
        ...
        map_odom.transform.translation.x = self.trans_map_odom.transform.translation.x
        map_odom.transform.translation.y = self.trans_map_odom.transform.translation.y
        map_odom.transform.translation.z = self.trans_map_odom.transform.translation.z
        map_odom.transform.rotation.x = self.trans_map_odom.transform.rotation.x
        map_odom.transform.rotation.y = self.trans_map_odom.transform.rotation.y
        map_odom.transform.rotation.z = self.trans_map_odom.transform.rotation.z
        map_odom.transform.rotation.w = self.trans_map_odom.transform.rotation.w
        self.tf_broadcaster.sendTransform(map_odom)

        rot_map_to_odom_inv = quaternion_inverse([
            self.trans_map_odom.transform.rotation.x,
            self.trans_map_odom.transform.rotation.y,
            self.trans_map_odom.transform.rotation.z,
            self.trans_map_odom.transform.rotation.w
        ])
        ...
        odom_base.transform.translation.x = self.robot_pose.pose.position.x - self.trans_map_odom.transform.translation.x
        odom_base.transform.translation.y = self.robot_pose.pose.position.y - self.trans_map_odom.transform.translation.y
        odom_base.transform.translation.z = self.robot_pose.pose.position.z - self.trans_map_odom.transform.translation.z
